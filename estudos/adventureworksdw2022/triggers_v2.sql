...
as
begin
	if(ORIGINAL_LOGIN() = 'sa' and HOST_NAME() <> 'BRNB-MATHEUS') --> qualquer server fora do meu hostname não consegue acessar
	-- if(ORIGINAL_LOGIN() = 'sa' and HOST_NAME() = 'BRNB-MATHEUS') -> não consigo acessar pela minha máquina
	begin
		rollback
	end
end
